...
function CompareContent() {
  ...
  function SearchBox({ value, onChange, onSelect }: { value: string; onChange: (v: string) => void; onSelect: (id: string) => void }) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          {!leg1 ? (
            <div className="bg-slate-800/30 rounded-xl p-6 border border-dashed border-slate-600 text-center">
              ...
              <SearchBox value={search1} onChange={setSearch1} onSelect={(id) => loadDetail(id, 1)} />
            </div>
          ) : (
            <div>
        ...
        <div>
          {!leg2 ? (
            <div className="bg-slate-800/30 rounded-xl p-6 border border-dashed border-slate-600 text-center">
              <p className="text-slate-500 text-sm mb-3">議員を選択してください</p>
              <SearchBox value={search2} onChange={setSearch2} onSelect={(id) => loadDetail(id, 2)} />
            </div>
          ) : (
            <div>
              <div className="flex justify-end mb-2">
