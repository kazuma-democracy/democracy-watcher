...
export default function ComparePage() {
  ...
  function SearchBox({ value, onChange, onSelect, slot }: { value: string; onChange: (v: string) => void; onSelect: (id: string) => void; slot: 1 | 2 }) {
        ...
        {candidates.length > 0 && (
          <div className="absolute top-full left-0 right-0 bg-slate-800 border border-slate-600 rounded-xl mt-1 z-20 max-h-60 overflow-y-auto">
            {candidates.map(c => (
              <button
                key={c.id}
                onClick={() => { onSelect(c.id); onChange('') }}
      ...
      <div>
        ...
        <div className={`rounded-xl overflow-hidden border border-slate-700/50 mb-4`}>
          ...
          <div className="bg-slate-800 p-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between"><span className="text-slate-500">所属院</span><span className="text-slate-200">{leg.house}</span></div>
            {leg.current_position && <div className="flex justify-between"><span className="text-slate-500">役職</span><span className="text-amber-400">{leg.current_position}</span></div>}
        ...
        <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
          ...
          {leg.monthly.length > 0 && (
            <div className="flex justify-between text-xs text-slate-600 mt-1">
              <span>{leg.monthly[0].month}</span>
              <span>{leg.monthly[leg.monthly.length - 1].month}</span>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          {!leg1 ? (
            <div className="bg-slate-800/30 rounded-xl p-6 border border-dashed border-slate-600 text-center">
              ...
              <SearchBox value={search1} onChange={setSearch1} onSelect={(id) => loadDetail(id, 1)} slot={1} />
            </div>
          ) : (
            <div>
        ...
        <div>
          {!leg2 ? (
            <div className="bg-slate-800/30 rounded-xl p-6 border border-dashed border-slate-600 text-center">
              <p className="text-slate-500 text-sm mb-3">議員を選択してください</p>
              <SearchBox value={search2} onChange={setSearch2} onSelect={(id) => loadDetail(id, 2)} slot={2} />
            </div>
          ) : (
            <div>
              <div className="flex justify-end mb-2">
      ...
      {loadingDetail && (
        <div className="text-center py-8">
          <div className="animate-pulse text-slate-400">データを取得中...</div>
        </div>
      )}
    </div>
  )
